...
void output(vector<pair<char, int>> entropyVector, string CPUcount, int cpu, pthread_mutex_t *mutex, pthread_mutex_t *mutex2, pthread_cond_t *condition, int *counter)
{
...
void* threadInstruct(void* arg)
{
    ...
    output(threadArg.entVec, threadArg.inpStr, threadArg.ord, threadArg.mutex, threadArg.mutex2, threadArg.condition, threadArg.counter);
    return NULL;
}
